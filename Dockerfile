...
# to be re-compiled.
COPY config/config.exs config/config.secret.exs config/${MIX_ENV}.exs config/
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/sue ./

USER nobody

CMD ["/app/bin/sue", "start"]
